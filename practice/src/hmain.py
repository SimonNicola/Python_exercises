...
def add_sentence(sentence_list: list[dict[str, str | int]]) ->  None:
    ...
    """
    text : str = input("Dime la frase")
    lan : str = input("Dime el lenguage Ejemplo: en, es...")
    vowels_ : int = 0
    consonants_: int = 0
    others: int = 0
    consonants_li : list[str] = ("B", "C", "D", "F", "G", "H", "J", "K", "L", "M", "Ñ", "P", "Q", "R", "S", "T", "V", "W", "X", "Y", "Z")
    vowels_li : list[str] = ("A", "E", "I", "O", "U")
    
    for char in text:
        if char.upper() in consonants_li:
            consonants_ += 1
        elif char.upper() in vowels_li:
            vowels_ += 1
        else:
            others += 1
